...
bool hasPerfectMatchingHelper( Map<string, Set<string>> &possibleLinks, Set<Pair>& matching){
    ...
    int peopleCount = possibleLinks.size();
    
    if(peopleCount == 0) return true; 

    if(peopleCount%2)   return false;

    auto p1 = possibleLinks.firstKey();
    Set<string> p1Link;
    p1Link += possibleLinks[p1];

    for(auto p2: p1Link){
        int flag = 0;
        for(auto pair: matching){
            if(p2==pair.first()||p2==pair.second()){
                flag = 1;
                break;
            }
        }
        if(flag) continue;
        auto p2Link = possibleLinks[p2];

        Pair greek(p1, p2);

        matching.add(greek);
        
        possibleLinks.remove(p1);
        possibleLinks.remove(p2);

        if(hasPerfectMatchingHelper(possibleLinks, matching)){
            return true;
        }

        possibleLinks[p1]=p1Link;
        possibleLinks[p2]=p2Link;
...
bool hasPerfectMatching(const Map<string, Set<string>>& possibleLinks, Set<Pair>& matching) {
    Map<string, Set<string>> _possibleLinks;
    _possibleLinks += possibleLinks;

    return hasPerfectMatchingHelper(_possibleLinks, matching);
}

int calculateWeight(const Map<string, Map<string, int>>& possibleLinks, Set<Pair>& pairs){
    int sum = 0;
    for(auto pair: pairs){
        string p1 = pair.first();
        string p2 = pair.second();
        Map<string, int> p1Link = possibleLinks.get(p1);
        for(auto p1Partner:p1Link){
            if(p2==p1Partner){
                sum += p1Link[p2];
            }
        }
...
Set<Pair> maximumWeightMatchingHelper(Map<string, Map<string, int>> &tempPossibleLinks, const Map<string, Map<string, int>>& allPossibleLinks, Set<Pair>& matching){
    Set<Pair> res;
    Set<Pair> temp;
    int maxWeight = -1;
    if(tempPossibleLinks.size()<1){
        return res;
    }
    auto p1Name = tempPossibleLinks.firstKey();
    Map<string, int> p1Link;
    p1Link += tempPossibleLinks[p1Name];
    for(auto p2Name: p1Link){
        int flag = 0;
        for(auto pair: matching){
            if(p2Name==pair.first()||p2Name==pair.second()){
                flag = 1;
                break;
            }
        }
        ...
        auto p2Link = tempPossibleLinks[p2Name];

        Pair greek(p1Name,p2Name);

        matching.add(greek);

        tempPossibleLinks.remove(p1Name);
        tempPossibleLinks.remove(p2Name);

        Map<string, Map<string, int>> p1PartnerLinks;
        Map<string, Map<string, int>> p2PartnerLinks;

        for(auto partnerName: p1Link){
            if(partnerName != p2Name){
                auto partnerLinks = tempPossibleLinks[partnerName];
                p1PartnerLinks[partnerName]=partnerLinks;
                partnerLinks.remove(p1Name);
                if(partnerLinks.isEmpty())
                    tempPossibleLinks.remove(partnerName);
                else{
                    tempPossibleLinks[partnerName]=partnerLinks;
                }
            }
        }

        for(auto partnerName: p2Link){
            if(partnerName != p1Name){
                auto partnerLinks = tempPossibleLinks[partnerName];
                p2PartnerLinks[partnerName]=partnerLinks;
                partnerLinks.remove(p2Name);
                if(partnerLinks.isEmpty())
                    tempPossibleLinks.remove(partnerName);
                else{
                    tempPossibleLinks[partnerName]=partnerLinks;
        ...
        temp = maximumWeightMatchingHelper(tempPossibleLinks, allPossibleLinks, matching);
        temp.add(greek);

        int tempWeight = calculateWeight(allPossibleLinks, temp);
        ...
        for(auto partnerName: p1PartnerLinks){
            tempPossibleLinks[partnerName]=p1PartnerLinks[partnerName];
        }

        for(auto partnerName: p2PartnerLinks){
            tempPossibleLinks[partnerName]=p2PartnerLinks[partnerName];
        }

        tempPossibleLinks[p1Name]=p1Link;
        tempPossibleLinks[p2Name]=p2Link;
        matching.remove(greek);
    }
    tempPossibleLinks.remove(p1Name);
    for(auto partnerName: p1Link){
        ...
        if(partnerLinks.isEmpty()){
            tempPossibleLinks.remove(partnerName);
        }else tempPossibleLinks[partnerName]=partnerLinks;
    }
    auto withoutP1 = maximumWeightMatchingHelper(tempPossibleLinks, allPossibleLinks, matching);
    int tempWeight = calculateWeight(allPossibleLinks, withoutP1);
    if(tempWeight>maxWeight) {
        maxWeight = tempWeight;
        res = withoutP1;
    }
...
Set<Pair> maximumWeightMatching(const Map<string, Map<string, int>>& possibleLinks) {
    Set<Pair> res;
    Set<Pair> matching;
    Map<string, Map<string, int>> tempPossibleLinks = possibleLinks;
    res = maximumWeightMatchingHelper(tempPossibleLinks, possibleLinks, matching);
    return res;
}
...
PROVIDED_TEST("hasPerfectMatching works on a caterpillar.") {
     ...
     * paired and who isn't.
     */
    Vector<string> people = { "B", "F", "C", "A", "E", "D" };
    // do {
    Map<string, Set<string>> links = fromLinks({
                                                { people[0], people[1] },
                                                { people[1], people[2] },
