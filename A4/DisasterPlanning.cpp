...
bool canBeMadeDisasterReadyHelper(const Map<string, Set<string>>& roadNetwork, int numCities, Set<string> &supplyLocations, Set<string>& unCovered, const Set<string>& citySet){
    if(numCities<=0&&!unCovered.isEmpty()){
        return false;
    }
    if(unCovered.isEmpty()) return true;

    auto covered = citySet;
    covered -= unCovered;

    auto c1 = unCovered.first();

    auto c1Links = roadNetwork[c1];

    for(auto c2: c1Links){
        supplyLocations.add(c2);
        if(!covered.contains(c2))
            unCovered.remove(c2);
        auto c2Links = roadNetwork[c2];
        for(auto c3: c2Links){
            if(!covered.contains(c3))
                unCovered.remove(c3);
        }

        if(canBeMadeDisasterReadyHelper(roadNetwork, numCities-1, supplyLocations, unCovered, citySet)){
            return true;
        }
        supplyLocations.remove(c2);
        if(!covered.contains(c2))
            unCovered.add(c2);
        for(auto c3: c2Links){
            if(!covered.contains(c3))
...
bool canBeMadeDisasterReady(const Map<string, Set<string>>& roadNetwork,
    ...
    if(numCities<0) error("The numCities shouldn't less than 0");
    ...
    return canBeMadeDisasterReadyHelper(roadNetwork, numCities, supplyLocations, unCovered, citySet);
}
