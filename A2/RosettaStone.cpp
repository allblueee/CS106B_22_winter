...
Map<string, double> kGramsIn(const string& str, int kGramLength) {
    /* Accept a str, return the result map
     * if the kGramLength <=0, call the error function()
     * if the length of str <= kGramLength, return an empty map
     */
    if(kGramLength<=0) {
        error("Oops, negative\n");
    }
...
Map<string, double> normalize(const Map<string, double>& input) {
    /* Receive a map, return the normalized result
     */
    if(input.size()==0){
        error("input map size is zero!\n");
    }
    bool flag = true;
    double squaresOfAll=0;
    for(auto e: input){
        if(input[e]!=0) flag=false;
        squaresOfAll+=pow(input[e], 2);
    }
    if(flag) error("The map doesn't contain at least one nonzero value.\n");
    double squaresRoot = sqrt(squaresOfAll);
    Map<string, double> res;
    for(auto e: input){
        res[e] = input[e]/squaresRoot;
    }
    return res;
}

Map<string, double> topKGramsIn(const Map<string, double>& source, int numToKeep) {
    /* Receive a source map and numTokeep, return the filtered map.
     */
    if(numToKeep < 0) error("Negative numToKeep.\n");
    Map<string, double> res;
    if(numToKeep == 0) return res;
...
double cosineSimilarityOf(const Map<string, double>& lhs, const Map<string, double>& rhs) {
    /* Receive two maps, return the cosineSimilar of them.
     */
    double res = 0;
    for(auto e: lhs){
        if(rhs.containsKey(e)){
            res += lhs[e]*rhs[e];
        }
    }
    return res;
...
string guessLanguageOf(const Map<string, double>& textProfile,
     ...
     * this function.
     */
    if(corpora.size()==0) error("The corpora set is empty.");
    string res;
    double max = 0;
    double consineSimilarity;
    for(auto e: corpora){
        consineSimilarity = cosineSimilarityOf(textProfile, e.profile);
        if(consineSimilarity > max){
            res = e.name;
            max = consineSimilarity;
        }
    }
    return res;
}
