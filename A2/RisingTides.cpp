...
void check(int row,
    ...
    if(row>=0&&row<numRow&&col>=0&&col<numCol){
        if(terrain[row][col]<=height&&!res[row][col]){
            GridLocation Location(row, col);
            q.enqueue(Location);
            res.set(row,col,true);
        }
    }
    return;
}
...
Grid<bool> floodedRegionsIn(const Grid<double>& terrain,
    ...
    int numRow = terrain.numRows();
    int numCol = terrain.numCols();
    Grid<bool> res(numRow,numCol);
    res.fill(false);
    Queue<GridLocation> q;
    int row, col;
    for(auto e : sources){
    ...
    while(!q.isEmpty()){
        ...
        row = Location.row;
        col = Location.col;
        check(row+1,col,numRow,numCol,terrain,height,q,res);
        check(row-1,col,numRow,numCol,terrain,height,q,res);
        check(row,col+1,numRow,numCol,terrain,height,q,res);
        check(row,col-1,numRow,numCol,terrain,height,q,res);
    }
    return res;
}
