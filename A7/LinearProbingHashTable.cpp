...
LinearProbingHashTable::LinearProbingHashTable(HashFunction<string> hashFn) {
    hashTableLength = hashFn.numSlots(); 
    elems = new Slot [hashTableLength];
    this->hashFn = hashFn;
...
bool LinearProbingHashTable::isEmpty() const {
    if(hashTableSize) return false;
    else return true;
}

bool LinearProbingHashTable::insert(const string& elem) {
    if(hashTableSize==hashTableLength) return false;

    int tempHashValue = hashFn(elem);

    int firstTombStone = -1;
    int start = tempHashValue;
    do {
        if(checkSlotState(tempHashValue) == SlotType::FILLED){
            if(elems[tempHashValue].value == elem) return false;
        }else{
            if(firstTombStone == -1) firstTombStone = tempHashValue;
        }
        tempHashValue = nextSlot(tempHashValue);
        if(tempHashValue == start) break;
        continue;
    } while(checkSlotState(tempHashValue) != SlotType::EMPTY);
    if(firstTombStone != -1)
        tempHashValue = firstTombStone;
    elems[tempHashValue].value = elem;
    elems[tempHashValue].type = SlotType::FILLED;
    hashTableSize++;
    return true;
}
...
bool LinearProbingHashTable::contains(const string& elem) const {
    int tempHashValue = hashFn(elem);
    if(checkSlotState(tempHashValue) != SlotType::EMPTY){
        if(checkSlotState(tempHashValue)==SlotType::FILLED && elems[tempHashValue].value == elem) return true;
        int start = tempHashValue;
        tempHashValue = nextSlot(tempHashValue);
        while(1){
            if(tempHashValue == start) return false;
            if(checkSlotState(tempHashValue) == SlotType::EMPTY) return false;
            if(checkSlotState(tempHashValue) == SlotType::TOMBSTONE) {
                tempHashValue = nextSlot(tempHashValue);
                continue;
            }
            if(elems[tempHashValue].value == elem) {
                return true;
            }else tempHashValue = nextSlot(tempHashValue);
        }
    } else return false;
}

bool LinearProbingHashTable::remove(const string& elem) {
    if(hashTableSize == 0) return false;

    int tempHashValue = hashFn(elem);
    if(checkSlotState(tempHashValue) != SlotType::EMPTY){
        if(checkSlotState(tempHashValue) == SlotType::FILLED && elems[tempHashValue].value == elem) {
            elems[tempHashValue].type = SlotType::TOMBSTONE;
            hashTableSize --;
            return true;
        }
        int start = tempHashValue;
