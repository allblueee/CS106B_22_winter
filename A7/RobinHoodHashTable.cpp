...
bool RobinHoodHashTable::insert(const string& elem) {
    ...
    int time = 0;
    while(1){
        if(time == hashTableLength) return false;
        time ++;
        if(elems[tempHashValue].distance != EMPTY_SLOT) {
            if(elems[tempHashValue].value == tempElem) return false;
        } else {
            elems[tempHashValue].value = tempElem;
            elems[tempHashValue].distance = i;
            hashTableSize++;
            return true;
        }
        if(elems[tempHashValue].distance == i){
            if(elems[tempHashValue].value == tempElem) return false;
        ...
        if(elems[tempHashValue].distance < i){
            swap(elems[tempHashValue].value, tempElem);
            swap(elems[tempHashValue].distance, i);
            tempHashValue = nextSlot(tempHashValue);
            i = nextSlot(i);
            continue;
        }
...
bool RobinHoodHashTable::contains(const string& elem) const {
    ...
    while(1){
        ...
        if(elems[tempHashValue].distance == EMPTY_SLOT){
            ...
            if(elems[tempHashValue].distance < i) return false;
            if(elems[tempHashValue].distance == i) {
                if(elems[tempHashValue].value == tempElem) {
                    return true;
                } else {
                    tempHashValue = nextSlot(tempHashValue);
                    i = nextSlot(i);
...
bool RobinHoodHashTable::remove(const string& elem) {
    ...
    if(deleteLoc != -1) {
        int endLoc = nextSlot(deleteLoc);
        int length = 0;
        while(!(elems[endLoc].distance==EMPTY_SLOT || elems[endLoc].distance == 0)){
            endLoc = nextSlot(endLoc);
            length++;
        }
        for(int i = 1;i<=length;i++){
            int prev = (deleteLoc + i - 1)%hashTableLength;
            int cur = (deleteLoc + i)%hashTableLength;
            elems[prev].distance = elems[cur].distance - 1;
            elems[prev].value = elems[cur].value;
        }
        endLoc = (endLoc - 1 + hashTableLength)%hashTableLength; 
        elems[endLoc].distance = EMPTY_SLOT;
        hashTableSize--;
        return true;
    }
    return false;
