...
bool checkOverlaps(Shift &it, Set<Shift> &resSet){
    int flag = false;
    for(auto re:resSet){
        if(overlapsWith(it,re)){
            flag = true;
            break;
        }
    }
...
int calculate(Set<Shift>& shifts){
    int total = 0;
    for(auto e: shifts){
        total += valueOf(e);
    }
...
/* Recursively enumeration every state
 *
 */
void Rec(Set<Shift> shifts, int maxHours, Set<Shift>resSet,Set<Set<Shift>>&alternative)
{
     if(maxHours<0){
        error("error: maxHours cannot less than 0.");
    }
    if(!resSet.isEmpty()) alternative.add(resSet);
    if(shifts.isEmpty()) return;
    auto it = shifts.first();
    shifts.remove(it);
    Rec(shifts, maxHours,resSet, alternative);
    if(!(lengthOf(it)>maxHours||checkOverlaps(it, resSet))){ 
        resSet.add(it);
        Rec(shifts, maxHours-lengthOf(it),resSet, alternative);
    }
    return; 
}

Set<Shift> Rec2(Set<Shift> shifts, int maxHours, Set<Shift>resSet){
    if(maxHours<0) error("Error maxHours should greater than 0");
    if(shifts.isEmpty()) return resSet;

    auto first = shifts.first();
    shifts.remove(first);

    Set<Shift> deletedShifts;
    deletedShifts += shifts;
    
    shifts += first;
    
    if(checkOverlaps(first, resSet)||lengthOf(first)>maxHours){
        return Rec2(deletedShifts, maxHours, resSet);
    }

    auto x = Rec2(deletedShifts, maxHours, resSet);
    resSet += first;
    auto y = Rec2(deletedShifts, maxHours-lengthOf(first), resSet);
...
/*    Set<Shift> temp;
    temp += shifts;
    Set<Shift> resSet;
    Set<Set<Shift>> alternative;
    Rec(temp, maxHours, resSet,alternative); 
    int max = 0;
    for(auto e: alternative){
        int temp = calculate(e);
        if(temp>max){
            resSet = e;
            max = temp;
        }
...
*/ 
    Set<Shift> resSet;
    return Rec2(shifts, maxHours, resSet);
}
