...
Map<string, int> apportion(const Map<string, int>& populations, int numSeats) {
    Map<string, int> res;
    int stateNum = populations.size();
    if(stateNum > numSeats) {
        error("The numSeats shouldn't less than the number of states");
    }
    HeapPQueue pq;
    for(auto stateName: populations){
        res[stateName]=1;
        DataPoint d;
        d.name = stateName;
        d.weight = -populations[stateName]/sqrt(2);
        pq.enqueue(d);
    }
    numSeats -= stateNum;
    while(numSeats > 0){
        ...
        res[d.name]++;
        d.weight = -populations[d.name]/(sqrt(res[d.name]*(res[d.name]+1)));
        pq.enqueue(d);
        numSeats--;
    }
    return res;
