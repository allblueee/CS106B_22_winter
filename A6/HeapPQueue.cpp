...
void HeapPQueue::enqueue(const DataPoint& data) {
    if(logicalSize==allocatedSize-1){
        ...
        for(int i=1;i<=logicalSize;i++){
            temp[i].name = elems[i].name;
            temp[i].weight = elems[i].weight;
        }
        delete [] elems;
        elems = temp;
    }
    ...
    int loc = logicalSize;
    elems[loc] = data;
    while(data.weight < elems[loc/2].weight && loc>1){
        swap(elems[loc], elems[loc/2]);
        loc /= 2;
    }
...
DataPoint HeapPQueue::dequeue() {
    if(logicalSize == 0) {
        ...
        while(1){
            int lChild = 2*loc;
            int rChild = 2*loc + 1;
            if(lChild <= logicalSize && rChild <= logicalSize){
                if(elems[loc].weight>min(elems[lChild].weight, elems[rChild].weight)){
                    int target = loc;
                    if(elems[target].weight>elems[lChild].weight){
                        target = lChild;
                    }
                    if(elems[target].weight>elems[rChild].weight){
                        target = rChild;
                    }
                    swap(elems[loc], elems[target]);
                    loc = target;
                    continue;
                } else break;
            } else if(lChild <= logicalSize){
                if(elems[loc].weight > elems[lChild].weight){
                    swap(elems[loc], elems[lChild]);
                    loc = lChild;
                }
                break;
            } else break; 
